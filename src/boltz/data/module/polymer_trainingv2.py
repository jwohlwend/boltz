...
@dataclass
class DatasetConfig:
    """Polymer-only dataset configuration (SMILES + properties)."""

    target_dir: str
    sampler: Sampler
    cropper: Cropper
    prob: float = 1.0
    msa_dir: Optional[str] = None  # ignored; kept for config compatibility
...
@dataclass
class DataConfig:
    """Polymer-only data configuration."""
...
def load_input(record: Record, target_dir: Path) -> Input:
    """Load the given input data (no MSA)."""
    structure_npz = np.load(target_dir / "structures" / f"{record.id}.npz")
    structure = Structure(
        atoms=structure_npz["atoms"],
        bonds=structure_npz["bonds"],
        residues=structure_npz["residues"],
        chains=structure_npz["chains"],
        connections=structure_npz["connections"].astype(Connection),
        interfaces=structure_npz["interfaces"],
        mask=structure_npz["mask"],
    )

    # Empty MSA map for polymer-only
    msas: dict[str, None] = {}
...
class TrainingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        try:
            input_data = load_input(sample.record, dataset.target_dir)
        ...
        if len(tokenized.tokens) == 0:
            raise ValueError("No tokens in cropped structure.")
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        try:
            ...
            features = dataset.featurizer.process(
                ...
                binder_pocket_conditioned_prop=self.binder_pocket_conditioned_prop,
                binder_pocket_cutoff=self.binder_pocket_cutoff,
                binder_pocket_sampling_geometric_p=1.0,
...
class PolymerTrainingDataModule(pl.LightningDataModule):
    def __init__(self, cfg: DataConfig) -> None:
        super().__init__()
        self.cfg = cfg
        assert self.cfg.val_batch_size == 1, "Validation only works with batch size=1."

        if cfg.symmetries is not None and Path(cfg.symmetries).exists():
        ...
        for data_config in cfg.datasets:
            ...
            train_records = [
                record for record in train_records if all(f.filter(record) for f in cfg.filters)
            ]
